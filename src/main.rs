...
use rosc::{decoder::decode_udp, encoder, OscMessage, OscPacket, OscType};
use tracing::{debug, error, info, warn, Level};
use tracing_subscriber::FmtSubscriber; // For argument parsing
        ...
        if r_vis < NUM_ROWS && c_vis < NUM_COLS {
            ...
            if self.grid[r_vis][c_vis] != desired_velocity {
                debug!(
                    ...
                    note, r_vis, c_vis, self.grid[r_vis][c_vis], desired_velocity
                );
            ...
            warn!(
                "Attempted to send grid note out of bounds: r_vis={}, c_vis={}",
                r_vis, c_vis
            );
        }
    }
        ...
        if bank_idx < NUM_LFO_BANKS {
            ...
            if self.lfo_banks[bank_idx] != desired_velocity {
                debug!(
                    ...
                    note, bank_idx, self.lfo_banks[bank_idx], desired_velocity
                );
                if let Err(e) = conn.send(&[0x90, note, desired_velocity]) {
            ...
            warn!(
                "Attempted to send LFO bank note out of bounds: bank_idx={}",
                bank_idx
            );
        }
    }
        ...
        if bank_idx < NUM_EFFECT_BANKS {
            ...
            if self.effect_banks[bank_idx] != desired_velocity {
                debug!(
                    ...
                    note, bank_idx, self.effect_banks[bank_idx], desired_velocity
                );
                if let Err(e) = conn.send(&[0x90, note, desired_velocity]) {
            ...
            warn!(
                "Attempted to send Effect bank note out of bounds: bank_idx={}",
                bank_idx
            );
        }
    }
...
impl AppState {
    fn new() -> Self {
        AppState {
            ...
            fader_override_active: Arc::new(RwLock::new(vec![
                vec![false; TOTAL_COLS];
                NUM_LFO_BANKS
            ])),
...
async fn main() -> Result<(), AppError> {
    ...
    if let Err(e) = led_tx.try_send(LedUpdateRequest::BothRefresh) {
        warn!(
            "Failed to send initial BothRefresh LED update request: {}",
            e
        );
    }
            ...
            if let Ok(lfo_source_on_grid) = row_str.parse::<usize>() {
                ...
                if lfo_source_on_grid >= 1 && lfo_source_on_grid <= NUM_ROWS {
                    ...
                    if let Some(OscType::Float(value)) = msg.args.get(0) {
                        ...
                        let mut latest_lfo_values_guard =
                            app_state.latest_lfo_values.write().unwrap();
                        ...
                        warn!(
                            "LFO message did not contain a float argument: {:?}",
                            msg.args
                        );
                    }
                } else {
                        ...
                        for packet in bundle.content {
                            match packet {
                                ...
                                OscPacket::Bundle(inner_bundle) => {
                                    warn!(
                                        "Received nested OSC Bundle, not yet handled: {:?}",
                                        inner_bundle
                                    );
                                }
                            }
...
fn clear_all_leds(midi_out_conn: &mut MidiOutputConnection) {
    ...
    for note_to_clear in 0..96 {
        ...
        if let Err(e) = midi_out_conn.send(&[0x90, note_to_clear, LED_OFF]) {
            warn!(
                "Failed to send MIDI note {} during clear_all_leds: {}",
                note_to_clear, e
            );
        }
    }
...
fn _update_bank_select_leds(
    midi_out_conn: &mut MidiOutputConnection,
    ...
    for i in 0..NUM_LFO_BANKS {
        let velocity = if i == current_lfo_bank {
            LED_ORANGE
        } else {
            LED_OFF
        };
...
fn _refresh_grid_leds(
    ...
    let current_effect_bank = app_state.banks.current_effect_bank.load(Ordering::SeqCst);
    debug!(
        "Refreshing grid LEDs. LFO Bank: {}, Effect Bank: {}",
        current_lfo_bank, current_effect_bank
    );
    ...
    for r_vis in 0..NUM_ROWS {
        // Visual rows on APC (LFOs)
        for c_vis in 0..NUM_COLS {
            // Visual columns on APC (Effects)
        ...
        if status & 0xF0 == 0x90 {
            ...
            if (48..=55).contains(&cc_number) {
                ...
                if actual_col_idx_fader < TOTAL_COLS {
                    ...
                    fader_override_value_guard[current_lfo_bank][actual_col_idx_fader] =
                        fader_float_val;
                    debug!(
                        "Fader CC: Set val={} for actual_col(Effect)={}, lfo_bank={}",
                        fader_float_val, actual_col_idx_fader, current_lfo_bank
                    );

                    if let Err(e) = led_tx.try_send(LedUpdateRequest::FaderColumnRefresh {
                        actual_effect_idx: actual_col_idx_fader,
                    }) {
                        warn!(
                            "Failed to send FaderColumnRefresh LED update request: {}",
                            e
                        );
                    }
                } else {
...
fn _refresh_fader_column_leds(
    midi_out_conn: &mut MidiOutputConnection,
    app_state: &Arc<AppState>,
    actual_effect_idx_of_fader: usize,
    led_state: &mut LedState,
) {
    let current_lfo_bank = app_state.banks.current_lfo_bank.load(Ordering::SeqCst);
    let current_effect_bank = app_state.banks.current_effect_bank.load(Ordering::SeqCst);
    ...
    let c_vis_of_effect = if actual_effect_idx_of_fader >= current_effect_bank * NUM_COLS
        && actual_effect_idx_of_fader < (current_effect_bank + 1) * NUM_COLS
    {
    ...
    if let Some(c_vis) = c_vis_of_effect {
        // c_vis is the visual column of the effect/fader
        for r_vis in 0..NUM_ROWS {
            // Iterate through visual rows (LFOs)
            let actual_r_lfo_idx = current_lfo_bank * NUM_ROWS + r_vis;
            // actual_c_effect_idx is fixed for this fader's column: it's actual_effect_idx_of_fader

            let mut led_velocity = LED_OFF;

            if actual_r_lfo_idx < TOTAL_ROWS {
                // actual_effect_idx_of_fader is already known to be < TOTAL_COLS if c_vis_of_effect is Some
                let is_fader_override =
                    fader_override_active_guard[current_lfo_bank][actual_effect_idx_of_fader];

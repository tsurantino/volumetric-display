...
ABSL_FLAG(std::string, geometry, "16x16x16", "Width, height, and length of the display");
ABSL_FLAG(std::string, ip, "127.0.0.1", "IP address to listen for ArtNet packets");
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        if (sscanf(geometry.c_str(), "%dx%dx%d", &width, &height, &length) != 3) {
            throw std::runtime_error("Invalid geometry format. Use WIDTHxHEIGHTxLENGTH (e.g., 16x16x16).");
        }
        ...
        VolumetricDisplay display(width, height, length, ip, port, universes_per_layer);
        display.run();
